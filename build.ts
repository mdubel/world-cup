...
/**
 * Static assets that live alongside the JS/CSS bundle in r/www and py/www.
 * Each gets copied from a source path into every existing target directory
 * on every build (and once at startup of watch mode).
 */
const STATIC_ASSETS: { from: string; to: string }[] = [
  { from: "srcts/favicon.png", to: "favicon.png" },
];

function copyStaticAssets() {
  const targets: string[] = [];
  if (existsSync("r")) targets.push("r/www");
  if (existsSync("py")) targets.push("py/www");

  for (const target of targets) {
    mkdirSync(target, { recursive: true });
    for (const asset of STATIC_ASSETS) {
      if (!existsSync(asset.from)) continue;
      copyFileSync(asset.from, `${target}/${asset.to}`);
    }
  }
}
